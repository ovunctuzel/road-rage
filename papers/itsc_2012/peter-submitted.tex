...
\texttt{http://code.google.com/p/road-rage}}, making it easier\commentp{easier
is a comparitive word, so this should be either "easy" or "easier than...."}
for users to test out a number of agent behaviors and intersection policies in
a short time span.
...
modules is visualized in \commentp{all figure and section references should
have capital F and S respectively}\commentpk{I tend to use Fig.}figure
\ref{fig:arch}.
...
interprets these points as straight lines \commentp{line segments instead of
lines} and projects parallel lanes out using a fixed width. Turns are modeled
\commentp{approximated} with a single straight line \commentp{segment}
connecting the end of one lane to the start of another.
...
\begin{enumerate}
  ...
  \item Because turns are heuristically placed, the graph is often disconnected.
        ...
        geometric gaps, but correctly ensuring connectivity. \commentp{This paragraph was generally unclear, and makes it sound like you are removing huge chunks of the graph}
\end{enumerate}
...
number of lanes and typical speed limits. Heuristically enumerating the turns
at each intersection often misses common features like right turn-only lanes
and shared center left turn lanes. There are many circumstances where AORTA
simulates one intersection \commentp{makes it sound like aorta's fault} as
several in close proximity because roads do not geometrically line up, but
functionally they do. If OSM encoded this data, the the\commentp{extra the}
realism of the simulation would improve. Other simulators have identified
\commentp{"dealt with" instead of "identified"} similar issues before
...
\commentp{The images in this section need better labels, plus maybe print
better in black and white. For instance, the gridlock issue - since b/w does
not show red explicitly, maybe you should mark the loop in the image. In Fig
3a) the green arrowhead is hidden etc.}
...
Currently agents cannot change lanes. This causes a second \commentp{potential}
issue: gridlock.  Each agent maintains a safe following distance from the next.
If a lane is filled, then an agent may be forced to stop in an intersection
mid-turn and block other traffic. The default behavior prevents this by
refusing to start a turn before the lookahead engine guarantees no agent will
...
in figure \ref{fig:gridlock} in order to enter a lane adjacent to their
original lane.  Since these routes are still legal, it is desirable to prevent
or mend this problem without forcing different routes \commentp{the need for
forccing alternate routes could be clearer}. However, choosing alternative
...
ideas such \commentp{as} dynamic contra-flow \cite{ITSC11-hausknecht}.  There
will also be an effort to improve AORTA as a framework by including a
lane-changing model, preventing gridlock, and supporting more agents through
parallel computing.
