...
One advantage of AORTA is that it simulates traffic on existing infrastructure rather than contrived,
...
\begin{enumerate}
  ...
  \item The builder constructs turns between incoming and outgoing lanes at each
        vertex. The builder uses an angle heuristic to choose between left, right, straight,
...
\begin{enumerate}
  \item Introduce new agents into the map
  \item Update the position and velocity of agents based on their choice of
  acceleration in the previous step
  \item Check for collisions
  \item Allow each agent to choose a reaction for the next step
\end{enumerate}
...
disappearing from the map (when the agent is at rest and is done with its
...
\commentm{What's the difference between lane changing and merging?}
...
since it would be illegal to drive further anyway.
...
\commentd{http://sumo.sourceforge.net/pdf/mieec1.pdf gives their measure as
  agent updates / second. This is indep of dt and num of agents, assuming
  nothing grows nonlinearly with more agents. our initial result on my box is
  about 150,000 moves / second. so with 500 agents, runs 30x faster than
  reality. with 5,000, runs about 3x faster. with 50,000 (yes, this works!),
  runs at 1/3 the speed of reality (it takes 1 real second to simulate 0.3
  seconds)}
\commentm{I typically have 160k-170k agent steps per second on my machine.  The
	highest I have seen is upwards of 300k, but that was because most agents had
	gridlocked...}
...
and micro-tolling \commentd{need a cite}. There will also be an effort to
improve AORTA as a framework by including a lane-changing model and preventing
gridlock.
\commentm{Perhaps mention concurency plans (with TACC) to simulate arbitrarily large maps?}
...
traffic signals would vastly improve in those regions.
...
\commentd{Er, these don't look like they're alphabetically sorted.}
