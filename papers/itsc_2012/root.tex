...
\commentd{terminology sucks below. suggestions?}
...
\commentd{(generators and thread pool)}
...
guarantee connectedness, there are necessarily circuits in most maps. Instead, collisions are
checked after every agent has stepped forward in time, treating each time-step
as atomic. Queues remember the original order of agents before a step,
and afterwards can verify that the order is still legal, with new agents on the end
...
but there is currently one known flaw. This baseline behavior picks the fastest
...
Each constraint contributes to the max acceleration that will obey the limit, so
...
\commentd{hopefully we'll have a neater solution than this soon}
