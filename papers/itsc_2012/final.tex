...
next, so long as the agent could safely brake without danger of entering the
...
is desirable to prevent or mend this without forcing different routes.
However, choosing alternative moves (by changing lanes or rerouting) is an
important constituent of a sufficient condition for the liveness of a road
network cite{AAAI11-au}.
...
A particular focus of AORTA is intersection policies. To demonstrate how
...
\cite{JAIR08-dresner}. This policy groups agents with compatible turns together,
allowing new agents to join existing groups. To prevent one heavy direction of
traffic from hogging the intersection indefinitely, a timeout preemptively
cycles through reservations.
...
stop signs cause about 7 seconds lag, and traffic signals using the heuristic
described above cause 16 seconds. These results will vary with different
traffic levels and policy configurations, so the important demonstration is that
AORTA can be used as a framework for evaluating these policies.
