...
\commentd{consider re-grouping sections... maybe this is all 'methodology'?} \commentpk{I think it is fine as it is}
...
Machine \commentpk{cite a link to Scala here}. Scala provides the advantage of
functional programming constructs while still permitting imperative style.
Parts of AORTA, including extensions and clients, can easily be written in Java
or Scala. The software is
open-source\footnote{http://code.google.com/p/road-rage} and easily extensible
\commentpk{maybe "Extensions and clients built on top of AORTA can be written
in either Java or Scala, as the software is
open-source\footnote{\commentpk{needs more description}
http://code.google.com/p/road-rage} and easily extensible}. \commentpk {For
instance, the} The stop sign policy used in Section \ref{sec:results}'s
experiments \commentpk{Sec VI's experiments is awk, maybe "used in experiments
described in Sec VI"} is implemented in just $70$ \commentpk{remove \$\$ in
latex code, here and elsewhere} lines of code.

\commentpk{need to reference the architecture figure here. the text in the
figure also needs to be bigger}
...
rather than contrived, randomly \commentpk{"contrived, randomly" is a bit
weird, maybe "contrived or randomly"?} generated maps. Utilizing OpenStreetMaps
\commentpk{remove trailing s in OpenStreetMaps}, a community project offering
...
AORTA's format, which is also encoded with XML. \commentpk{why convert it to
another XML format? you might want to put an additional line outlining the
...
\commentpk{measuring?} physical distances. A road contains a sequence of points
giving the line segments of the center yellow line \commentpk{if you want to
use "yellow", you need to reference a figure. I would recommend putting it this
way "center line of the road (yellow lines in Fig. XX)"}. These points could
also be interpreted as control points for a curved road, but AORTA uses
straight lines as an approximation. Lanes are parallel to the center line, some
fixed width apart from one another \commentpk{no need to mention that the roads
could be interpreted as a spline, instead say "AORTA interprets roads as
straight lines between these points, and projects parallel lanes out using a
fixed width"}. Turns are modeled with a single straight line connecting the end
...
\begin{enumerate}
  \item OSM encodes many paths besides driveable roads, so the builder first filters
        these out. Next the builder \commentpk{you have not mentioned a builder before, maybe mention it in the last line of the intro to this section} marks GPS locations common to multiple roads, since
        they implicitly indicate intersections \commentpk{do you guys use GPS locations or OSM unique identifiers, what about overpasses etc.?}.
  ...
  \item The builder multiplies undirected roads into directed lanes in each direction
        ...
        OSM's ``road type'' \commentpk{If you have space, elaborate that you use heuristics on the road type attribute, and the 4 different road types are residential etc.} tag or an explicit number of lanes, when that data is available.
  \item The builder constructs turns between incoming and outgoing lanes at each
        junction \commentpk{intersection, keep the terminology constant}. The builder uses an angle heuristic to choose between left, right, straight,
        and U-turns \commentpk{preceding sentence unclear to me}. When several lanes all cross into fewer lanes,
        ...
        like Google Maps \cite{googleMaps} lack this information \commentpk{the last sentence is tricky. unless you have a source that states this, I would avoid this claim. Linking to google maps directly is not correct}.
...
space-continuous, meaning roads are not tiled \commentpk{recommend breaking up
preceding sentence into 2. I lost track haflway}. The step duration $dt$ is
fixed, enabling agents to reason about collision avoidance and satisfying other
constraints \commentpk{preceding sentence is unclear}. Setting this value to
the average ``reaction time'' of a human driver may be appropriate for some
\commentpk{"human driver" instead of some?} experiments. Each step, the
...
\begin{enumerate}
  ...
  \item Allows each agent to choose a reaction \commentpk {should be action} for the next step
\end{enumerate}
...
agent's behavior is responsible for obeying these \commentpk{if these "other"
constraints are not enforced, clearly specify. Something like "Agent behaviors
are responsible for following intersection policies as directed by an
intersection, and are responsible for not colliding with other agents" probably
sounds a bit better}. There are three main components of the simulation, each
configurable by extending built-in implementations of an interface: agent
behaviors, routing strategies, and intersection policies.
...
\commentpk{this section is a bit incoherent, and might need to be reorganized.
should the constraints be mentioned here explicitly, or maybe just added to
agent behaviors? What are you introducing in the next few subsections? I can
see A-E working together as an explanation of the workings of the simulator,
and would make more sense right after the enumeration above. Section G can be
moved into the experiments section. Once you are done with this primary
explanation, Section F should be talked about separately under something along
the lines of current limitations of the simulator.}
...
starting position and goal from two sets of roads \commentpk{preceding sentence
is ambiguous, maybe go with "The generator produces a desired amount of traffic
by initializing agents from random locations inside a \textit{start} polygon to
random destinations inside a \textit{goal} polygon, as demarcated by the
user"}. Users can select these roads by mouse with a bounding polygon in the
UI, allowing specific traffic patterns like rush-hour scenarios to be easily
simulated.  A generator \commentpk{polygon} may also cover the entire map,
uniformly distributing traffic everywhere.
...
sends it to a pool of worker threads to process in the background \commentpk{if multiple threads are available?}.
...
% TODO in fact, this is caused by lack of lane changing!!!
% but that's scary... bad routes really do cause it.
...
decides to pick a different turn. AORTA can detect this by finding cycles in the
graph of the ``is following'' relation.
...
When $dt = 0.1$ seconds, this means $15,000$ agents can be comfortably simulated
in real-time.  Further optimizations are planned.

\commentd{if i have a day after the paper's done, I can make this number
better.}
...
indefinitely, a timeout preempts and cycles through reservations. Ideally this
should be based on the relative numbers of agents depending on their
reservation.

\subsection{Results}

\pix{avg_lag_agent_atx.png}
    {The distribution of average delay reveals that traffic signals can cause the most lag.}
    {scale=0.35}
...
time is defined as lag and is used to measure intersection performance. On
average, reservation policies cause less than $1$ second of delay at each
intersection, stop signs cause about $7$ seconds lag, and traffic signals cause
$16$ seconds.

\pix{agent_cnt_atx.png}
    {Autonomous reservations outperform stop signs, and traffic signal gridlocked.}
    {scale=0.35}
...
introduces an agent at the same rate as older agents finish their route. As one
measure of intersection policy efficiency, it reveals that autonomous
reservations are twice as efficient as the competition, with a steady-state of
about $400$ agents. The count for traffic signals continues to increase because
gridlock occurred in one segment of the map, causing agents to not finish their
route. Unexpected starvation was also observed: although the signals cycle
through turns regularly, agents cannot proceed when the previous cycle caused a
