...
\cite{pursula1999simulation}. \commentpk{preceding sentence doesnt paraphrase
the ref correctly. double check}. Advanced computing techniques have enabled
...
have been introduced in the past decade. We review a number of such simulators
...
specifically for autonomous vehicles \cite{JAIR08-dresner}. Based on experience
with SUMO, AORTA also displays more robust handling of data exported directly
from OSM. \commentpk{I am worried about the last line. It might be true, but it
is a fairly strong claim - we should put a citation of us handling something
that has been documented as not being handled via SUMO} 
...
aims to optimize traffic flow of autonomous vehicles at a given intersection.
AIM studies problems at the scale of roughly a dozen intersections arranged in
a regular grid, while AORTA has the potential to apply some of the same
research at a city-wide scale to real-world road networks.
...
Section \ref{sec:map} and \ref{sec:simulation} describe map construction from
...
policy affects agent performance \commentd{performance is a vague term, but I
don't want to talk about dependent vars just yet}, three policies are evaluated: stop sign,
...
their group of reservations. \commentd{take out the last sentence? all of the
policies could use some work.}
...
cost \commentd{do we want to raise this issue?}, suggesting a mixed approach,
